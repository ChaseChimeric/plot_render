...
merged_df = pd.read_hdf('../datasets/gse_position_1996.h5')

file_data_95 = '../datasets/QTN_merged_1996.h5'
file_tnr_95 = "../datasets/TNR_results_1996.h5"
...
if inbound.index[0].month == outbound.index[-1].month:
    data_95_11 = pd.read_hdf(file_data_95, key = f"{int(pass_dat.index[0].month):02}").sort_index()
else:
    data_95_11_1 = pd.read_hdf(file_data_95, key = f"{int(pass_dat.index[0].month):02}").sort_index()
    data_95_11_2 = pd.read_hdf(file_data_95, key = f"{int(pass_dat.index[-1].month):02}").sort_index()
...
def update(frame):
    if frame % 2 == 0:
        print(f"\rProgress: {((float)(frame)/total_frames)*100}%", end='', flush=True)
    ...
    try:
        ...
        print("set y failed ax4, continuing",end="")
    #print("ax4 end")
